...
export class MemberHelpTicketController {
    ...
    @ApiOperation({ summary: 'Member: Submit a help ticket to admin' })
    async submitTicket(
    ...
    @UseGuards(RolesGuard)
    @AllowedRoles(Roles.Member)
    @ApiOperation({ summary: 'Member: List own submitted help tickets' })
    ...
    @UseGuards(RolesGuard)
    @AllowedRoles(Roles.Member)
    @ApiOperation({ summary: 'Member: Get a specific own help ticket by ID' })
