...
import { NotificationPreferenceResponseDto } from './dto/notification-preference.dto';
...
export class NotificationController {
    ...
    @AllowedRoles(Roles.Trainer, Roles.Member)
    @ApiOperation({ summary: 'Trainer / Member: get notification preferences' })
    ...
    @UseGuards(RolesGuard)
    @AllowedRoles(Roles.Trainer, Roles.Member)
    @ApiOperation({ summary: 'Trainer / Member: update notification preferences (PATCH — send only the fields to change)' })
    ...
    @HttpCode(200)
    @AllowPendingAccount()
    @ApiOperation({ summary: 'Register or refresh a Firebase Cloud Messaging device token' })
    ...
    @HttpCode(200)
    @AllowPendingAccount()
    @ApiOperation({ summary: 'Deactivate a Firebase Cloud Messaging device token' })
